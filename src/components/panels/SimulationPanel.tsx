...
import { X, Play, RotateCcw, ChevronLeft, ChevronRight, Zap, Flag, MessageSquare, HelpCircle, GitBranch } from 'lucide-react'
...
export function SimulationPanel() {
  ...
  const handleConditionBranch = useCallback((branch: 'true' | 'false') => {
    if (!currentNode || !currentQuest) return
          ...
          {!currentNode ? (
            ...
            <div className="space-y-4">
              ...
              {outgoingConnections.length === 0 && !isEndNode && (
                <div className="text-sm text-validation-warning bg-validation-warning/10 
  ...
  switch (node.type) {
    ...
    case 'DIALOGUE':
      return (
        <div className="space-y-2">
          {node.speaker && (
            <p className="text-xs font-medium text-accent-blue">{node.speaker}</p>
          )}
    ...
    case 'CHOICE':
      return (
        <div className="space-y-2">
          {node.prompt && (
            <p className="text-sm text-text-secondary italic">{node.prompt}</p>
          )}
          {node.options && node.options.length > 0 && (
            <div className="space-y-2">
              {node.options.map((option) => (
    ...
    case 'END':
      return (
        <div className="space-y-2">
          <h3 className="text-lg font-semibold text-text-primary">{node.title}</h3>
          <div className={`inline-block px-2 py-0.5 text-xs rounded ${
            ...
            <div className="pt-2">
              ...
              <ul className="text-sm text-text-primary space-y-1">
                {node.rewards.map((reward, i) => (
                  <li key={i}>• {reward.type}: {reward.value} {reward.quantity && `x${reward.quantity}`}</li>
                ))}
              </ul>
            </div>
          )}
